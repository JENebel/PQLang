...
namespace PQLang
{
    ...
    {
        private static readonly string[] keywords = { "while", "fun", "if", "else", "print", "sqrt", "for", "floor", "ceil" };
        ...
        private static (string program, string[] libs) PrepareProgram(string input)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                if (!Char.IsWhiteSpace(input[i]) || inString || (output.EndsWith("fun") && input[i]==' '))
                {
                    output += input[i];
                }
            }
        ...
        private static Expression ParseStatement(string statement)
        {
            ...
            if (statement.StartsWith("for("))
            {
                ...
                if (header.Length == 1) return new ForLoopExpression(
                    new AssignmentExpression("0", new PrimitiveExpression(new Number(0))), 
                    new BinaryExpression(new VariableLookupExpression("0"), Operator.LessThan, header[0]), 
                    new AssignmentExpression("0", new UnaryExpression(new VariableLookupExpression("0"), Operator.PlusPlus)), 
